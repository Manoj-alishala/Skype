...
const MessageInput = () => {
	...
	// Close emoji picker on conversation change
	useEffect(() => {
		setShowEmojiPicker(false);
		...
		try {
			...
			mediaRecorder.onstop = async () => {
				...
				reader.onloadend = async () => {
					...
					await sendMessage(null, null, base64Audio, recordingDuration);
	...
	const handleSubmit = async (e) => {
		...
		await sendMessage(message.trim() || null, imagePreview || null);
		setMessage("");
		removeImagePreview();
	};
			...
			{imagePreview && (
				<div className='mb-2 relative inline-block animate-fade-in'>
					...
					<button
						...
						className='absolute -top-2 -right-2 text-red-400 hover:text-red-300 bg-gray-900 rounded-full transition-colors'
					>
			...
			{isRecording ? (
				<div className='flex items-center gap-3 glass-input rounded-2xl px-4 py-2.5'>
					...
					<button
						...
						className='p-2 rounded-xl bg-red-500 hover:bg-red-600 text-white shadow-lg shadow-red-500/20 transition-all'
					>
				...
				<div className='flex items-center gap-2 glass-input rounded-2xl px-4 py-2 focus-within:border-primary-400/50 focus-within:ring-1 focus-within:ring-primary-400/20 transition-all'>
					...
					<button
						...
						className='p-1.5 rounded-lg transition-all duration-200 flex-shrink-0 text-gray-400 hover:text-primary-400 hover:bg-white/5'
					>
					...
					<input
						ref={inputRef}
						type='text'
						className='flex-1 bg-transparent text-sm text-white placeholder-gray-400 focus:outline-none py-1.5'
					...
					{/* Voice record button (show when no text/image) */}
					{!message.trim() && !imagePreview ? (
						<button
							type='button'
							onClick={startRecording}
							className='p-2 rounded-xl text-gray-400 hover:text-red-400 hover:bg-white/5 transition-all duration-200 flex-shrink-0'
						...
						<button
							...
							className={`p-2 rounded-xl transition-all duration-200 flex-shrink-0 ${
								message.trim() || imagePreview
									? 'bg-primary-500 hover:bg-primary-600 text-white shadow-lg shadow-primary-500/20 scale-100'
